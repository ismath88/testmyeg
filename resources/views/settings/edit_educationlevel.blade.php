  ...
  <div class="container add-education-page">
        <div class="fade-in">
            ...
            <div class="col-md-10 col-12 row ml-5 mt-5">
				...
				{{ csrf_field() }}
					<div class="">
                        ...
                        <div class="card-body">
                            <form action="" method="post">
                                <div class="form-group">
									...
									<input type="hidden" value={{  $result['edu_details']->id }}" name="id" >
									...
									<option value="{{  $value->id }}" {{ $result['edu_details']->mainlevel_id == $value->id  ? 'selected' : ''}}>{{  $value->level_name }}</option>
						            @endforeach
									</select>
                                </div>
								<div class="form-group">
									...
									<option value="{{  $value->id }}" {{ $result['edu_details']->country_id == $value->id  ? 'selected' : ''}}>{{  $value->country_name }}</option>
						            @endforeach
									</select>
                                </div>
                                <div class="form-group">
                                    <label for="nf-password">Title</label>
                                    <input class="form-control educationlevel_name"  type="text" value="{{ $result['edu_details']->educationlevel_name }}" name="educationlevel_name" autocomplete="educationlevel_name" required>
                                </div>
						...
						<div class="form-group">
                                    ...
                                    <label class="switch">
									  <input style="height:auto !important;" type="checkbox" {{ $result['edu_details']->status == 1  ? 'checked' : ''}} name='status'>
									  <span class="slider round"></span>
									</label>
                                </div> 
					<!--div class="custom-control custom-switch">
				  <input type="checkbox" class="custom-control-input checked" value="on">
				...
				<div class="col-md-6 mt-4 col-12"  style="display:none">
				<div class="col-12">
					<div>Grade: HD-N </div>
...
$( document ).ready(function() {
	 ...
	 $(".store_educationlevel").click(function(e)	{		
		...
		{
			var save_url ="{{ url('update_educationlevel') }}";
