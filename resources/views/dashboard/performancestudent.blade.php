        ...
        <div class="fade-in">
            ...
            <!-- START -->
				<div class="col-md-12 sub-header px-0 d-flex col-12">
					...
					<a class="select-btn header active" href="{{ url('university/performancestudent') }}">Student Enrollment</a>
					<a class="select-btn header" href="{{ url('university/salesandcommission') }}">Sales and Commissions</a>
				</div>
			...
			<div class="row mx-0 mb-3">
				<div class="col-md-3 col-lg-3 date-box uni-no-padd">
					<div class="date-inner">
						<div class="degree-text">Student Enrollments Sent</div>
						<div class="degree-number">{{$result['application_sent']}} <span><?= date('F Y') ?></span></div>
					</div>
				</div>
				<div class="col-md-3 col-lg-3 date-box uni-no-padd">
					<div class="date-inner">
						<div class="degree-text">Student Enrollments Rejected</div>
						<div class="degree-number">{{$result['application_rejected']}} <span><?= date('F Y') ?></span></div>
					</div>
				</div>
				<div class="col-md-3 col-lg-3 date-box uni-no-padd">
					<div class="date-inner">
						<div class="degree-text">Student Enrollments  Approved</div>
						<div class="degree-number">{{$result['application_approved']}} <span><?= date('F Y') ?></span></div>
					</div>
				</div>
				<div class="col-md-3 col-lg-3 date-box uni-no-padd">
					<div class="date-inner">
						<div class="degree-text">Student Enrollments  Income</div>
						<div class="reject-number text-left">  $<span class="approve_amt_val">{{$result['approved_application_amt']}}</span></div>  
						<div class="row pt-2 "><div class="col dates day_btn">Day<?php //echo date('Y-m-d') ?></div><div class="col dates  px-0 week_btn">Week<?php 
						...
						?></div>     
						<div class="col dates pr-0 month_btn active">Month<?php // echo date('F Y'); ?></div> <div class="col dates year_btn">Year<?php //echo date('Y') ?></div>
						</div>
			...
			<div class="row mx-0 mb-4 px-0 box-shadow-show">
				<div class="col-md-9 dash-app-county card mb-0 px-0">
					...
					<div class="d-flex px-4 mt-3 border-top">
						...
						<div class="container col-8 pt-4 row text-right mx-0" id="search_countrysresult">  
...
$( document ).ready(function() {
	...
	var mode = 'day';
		...
		$.ajax({
				type:"GET",
				url: 'get_studentapplication_amount_total',
				data: {mode: mode},
				success: function(data) {
					$(".approve_amt_val").html(data);
					$('.day_btn').addClass("active");
	...
	$('.pick_country_id').click(function(event) {
		...
		window.location.href = 'university/performancestudent?country_id=' + country_id;
	});
